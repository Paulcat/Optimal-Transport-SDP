...
scales_l = -8:2:2; L = length(scales_l);
scales_r = -8:2:2; R = length(scales_r);
...
for i=1:L
	...
	for j=1:R
		...
		% functionals
		[f,f0] = ot1_fobj		(mm,costF,c1,c2,	 la);
		g = ot1_fgrad	(mm,costF,c1,c2,f0,la);
		...
		for k=1:50
			v	= rand(prod(nn+1),1);
			v	= v/norm(v,'fro');
			normg = max(normg, norm(g(T,v) + 1/rho*Tpen_g(U,T,v),'fro') / norm(v,'fro'));
		end
		%
		fval (i,j) = f(T) + 1/rho*Tpen(U,T);
		gnorm(i,j) = normg;
