...
[Yf1,Xf1] = meshgrid(-n2:n2,-n1:n1);
c1 = exp(-2i*pi*( Xf1(:)*t1(:,1)' + Yf1(:)*t1(:,2)' )) * a1(:);
c1 = reshape(c1,2*[n1 n2]+1);
c1 = ifftshift(c1);
...
[Yf2,Xf2] = meshgrid(-n4:n4,-n3:n3);
c2 = exp(-2i*pi*( Xf2(:)*t2(:,1)' + Yf2(:)*t2(:,2)' )) * a2(:);
c2 = reshape(c2,2*[n3 n4]+1);
c2 = ifftshift(c2);
...
[Tpen,Tpen_g] = ffw_Tpen(nn+1);

% parameters
[la,rho] = deal(rand,rand);


[f,f0] = ot_fobj (nn+1,h,c1,c2,la); % TODO: why do I force n+1 here?
g		 = ot_fgrad(nn+1,h,c1,c2,f0,la);
%
f		 = @(u) f(Tprojn(nn+1,u));
g		 = @(u) 2*g(Tprojn(nn+1,u),u);
...
for i = 1:length(la_list)
	...
	for j = 1:length(rho_list)
		...
		[f,f0] = ot1_fobj (nn+1,h,c1,c2,la);
		[g] = ot1_fgrad(nn+1,h,c1,c2,f0,la);
		%
		f = @(u) f(Tproj(u)) + 1/rho * Tpen(u,Tproj(u));
		g = @(u,h) g(Tproj(u),h) + 1/rho * Tpen_g(u,Tproj(u),h);
		...
		%
		testf_r(i,j,:) = la*rho/(la+rho) * [f(U),f(U0)];
		testg_r(i,j,:) = la*rho/(la+rho) * [norm(g(U,v)), norm(g(U0,v))];
		%
