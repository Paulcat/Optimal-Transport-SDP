function [G,GU] = ot2_fgrad(m,cost,u1,u2,la,rho)
...
GT = @(T,h) 1/2/la * Tprod4(m,N.*(T.*I-Y),h) - 1/rho*Tprod4(m,T,h);

% gradient
G  = @(U,h)   f0*( Tprod4(m,N.*cost,h) + GT(Tproj4(m,U),h) + 1/rho*U*(U'*h) );
GU = @(T,U,h) f0*( Tprod4(m,N.*cost,h) + GT(T,h) + 1/rho*U*(U'*h) );
